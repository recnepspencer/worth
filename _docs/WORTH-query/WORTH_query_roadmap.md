...
progress and local dependency checks. Phases 2.1-2.6 then close the measured House
bottlenecks in order: linear effect preparation and preadmitted index publication;
liveness-bounded index generations/checkpoints; shared candidate preparation;
exact consumed-dependency output reuse with changed-component writes; managed
incremental scene views; and compatible capture/reopen under original limits.
Phase 3 adds complete control flow, coverage and outbound custody; Phase 4 adds
A/B coexistence, branch/fork and program migration;
