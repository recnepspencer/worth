use super::{
    relational::{self, CargoRecords},
    routing::{RouteGraph, INPUT},
};
...
/// The court uses bound correspondence and sealed owner services. Accidental
...
pub fn install(
    ...
    let candidate =
        BridgeSemanticDependencyCandidate::admit(BridgeSemanticDependencyCandidateParts {
            source_installation_identity: "supply-chain-v1".into(),
            source_basis: "cargo-schema-v1".into(),
            source_runtime_authority: profile.runtime_instance_id(),
            source_installation_generation: 1,
            source_authority_binding_identity: profile.adapter_semantic_identity().into(),
            ...
            graph_participation_identity: "supply-chain-routing".into(),
            graph_adapter_identity: profile.adapter_semantic_identity().into(),
        ...
        .expect("court declaration: semantic dependency");
    ...
    let TransitionOutcome::Success(node) = graph.admit_installed_node(nodes.source) else {
        panic!("court declaration: source node")
    };
    let TransitionOutcome::Success(slot) = graph.admit_installed_aspect(nodes.source, INPUT) else {
        panic!("court declaration: source aspect")
    };
