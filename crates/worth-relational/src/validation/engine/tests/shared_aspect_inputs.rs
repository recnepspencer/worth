...
fn overlapping_field_reads_are_shared_without_sharing_verdicts_or_changed_basis() {
    ...
    assert_shared_field_reads(&runtime, true);
    ...
    assert_shared_field_reads(&runtime, false);
}

fn assert_shared_field_reads(runtime: &RelationalRuntime, expect_violation: bool) {
    ...
    assert_eq!(counters.custom_invariant_candidate_entity_aspect_reads, 2);
    assert_eq!(counters.custom_invariant_candidate_relation_aspect_reads, 0);
    assert!(counters.custom_invariant_candidate_reuse_hits >= 6);
}
