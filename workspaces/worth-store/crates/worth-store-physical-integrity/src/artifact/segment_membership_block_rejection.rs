...
const LEAF_ENTRY_BYTES: u64 = 40;
const BRANCH_REFERENCE_BYTES: u64 = 56;

pub(super) fn scope_mismatch(
    scope: PhysicalArtifactScope,
    block: &PhysicalSegmentMembershipBlock,
) -> Option<PhysicalIntegrityRejection> {
    let expected = scope.segment_membership_block_identity().unwrap();
...
fn range_mismatch(
    ...
    let (offset, length) = if observed.first() != expected.first() {
        first_range(block)
...
fn first_range(block: &PhysicalSegmentMembershipBlock) -> (u64, u64) {
...
fn last_range(block: &PhysicalSegmentMembershipBlock) -> (u64, u64) {
    if let Some(entries) = block.entries() {
        ...
        (
            BODY_OFFSET
                + (block.children().unwrap().len() as u64 - 1) * BRANCH_REFERENCE_BYTES
                + 40,
            16,
        )
    }
...
pub(super) fn membership_denial(
    scope: PhysicalArtifactScope,
    bytes: &[u8],
    denial: BoundedSegmentMembershipBlockDecodeDenial,
) -> PhysicalIntegrityRejection {
    match denial {
        BoundedSegmentMembershipBlockDecodeDenial::Format(denial) => {
            format_denial(scope, bytes, denial)
        }
        BoundedSegmentMembershipBlockDecodeDenial::LeafEntries { .. }
        | BoundedSegmentMembershipBlockDecodeDenial::BranchChildren { .. } => count_damage(scope),
    }
}
...
fn format_denial(
    scope: PhysicalArtifactScope,
    bytes: &[u8],
    denial: SegmentMembershipBlockDenial,
) -> PhysicalIntegrityRejection {
    match denial {
        SegmentMembershipBlockDenial::Frame(denial) => from_frame_denial(scope, denial),
        SegmentMembershipBlockDenial::MalformedPrefix => malformed_prefix(scope, bytes),
        SegmentMembershipBlockDenial::IdentityOrCapacity => identity_or_capacity(scope, bytes),
        SegmentMembershipBlockDenial::LevelOrKind => level_or_kind(scope, bytes),
        SegmentMembershipBlockDenial::MalformedLength => count_damage(scope),
        SegmentMembershipBlockDenial::InvalidEntry => body_damage(
            scope,
            PhysicalDamageCause::MalformedStructure,
            PhysicalFormatField::Payload,
        ),
        SegmentMembershipBlockDenial::InvalidReference => reference_damage(scope, bytes),
        SegmentMembershipBlockDenial::CanonicalOrder if read_u16(bytes, 64) == 0 => body_damage(
            scope,
            PhysicalDamageCause::MalformedStructure,
            PhysicalFormatField::Payload,
        ),
        SegmentMembershipBlockDenial::CanonicalOrder => reference_damage(scope, bytes),
    }
}
