...
use worth_store_physical_integrity::{
    ...
    PhysicalIntegrityRejectionClass, PhysicalWorkObligationIntegrityValidation,
    UntrustedPhysicalArtifact,
};

pub const OBLIGATION_OFFSET: u64 = 8_192;

pub fn store(bytes: [u8; 16]) -> StableStoreIdentity {
...
pub fn identity(runtime: u64, generation: u64, operation: u64) -> PhysicalWorkObligationIdentity {
    PhysicalWorkObligationIdentity::new(
        NonZeroU64::new(runtime).unwrap(),
        NonZeroU64::new(generation).unwrap(),
        NonZeroU64::new(operation).unwrap(),
    )
}
...
pub fn scope(
    ...
    PhysicalArtifactScope::physical_work_obligation(
        store,
        identity,
        PhysicalByteRange::new(OBLIGATION_OFFSET, 160).unwrap(),
    )
}
...
pub fn rejection(
    ...
    match validate_physical_work_obligation(
        UntrustedPhysicalArtifact::from_bounded_bytes(bytes),
        scope,
    ) {
        (PhysicalWorkObligationIntegrityValidation::Rejected(rejection), counters) => {
            (rejection, counters)
        }
        (PhysicalWorkObligationIntegrityValidation::Intact(_), _) => {
            panic!("damaged physical-work obligation unexpectedly validated")
        }
    }
}
...
pub fn reseal(bytes: &mut [u8; 160]) {
    let digest = independent_sha256(&bytes[..128]);
    bytes[128..160].copy_from_slice(&digest);
}

pub fn independent_sha256(input: &[u8]) -> [u8; 32] {
    const INITIAL: [u32; 8] = [
    ...
    let bit_len = u64::try_from(input.len()).unwrap().checked_mul(8).unwrap();
...
fn compress(state: &mut [u32; 8], words: &[u32; 64], constants: &[u32; 64]) {
    ...
    for index in 0..64 {
        ...
        let second = upper_a.wrapping_add((a & b) ^ (a & c) ^ (b & c));
        h = g;
        g = f;
        f = e;
