...
fn c9_journal_process_courtroom() {
    ...
    if std::env::var_os("WORTH_C9_RETAIN_WORLD").is_some() {world.disable_cleanup(true);}
    ...
    let producer=run_subject(&executable,&reports,"producer",ProcessSubjectRequest::producer(
        scenario,run,root.clone(),reports.join("producer.report"),
    ));
    let ProcessReportPayload::Produced(manifest)=producer.report.payload() else {panic!("actual producer")};
    producer.report.require(RootWireRole::Producer,scenario,run,manifest.store_identity(),
        producer.process_id,executable_sha256(&executable).unwrap()).unwrap();
    manifest.require_unchanged(&root).unwrap();
    super::artifact_courtroom::run_journals(std::path::Path::new(&observer),&root,&stores,&reports);
}
