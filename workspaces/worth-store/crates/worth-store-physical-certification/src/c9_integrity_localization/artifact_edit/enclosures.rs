...
use super::{frame_checksum, ArtifactInventory};
...
pub(super) fn graph(inventory: &ArtifactInventory, baseline: &Path) -> Vec<Edge> {
    ...
    for (parent, granule) in inventory.granules.iter().enumerate() {
        ...
        match granule.family {
            "root_manifest" => {
                for (flag, offset, family) in [
                    (88, 96, "root_routing_block"),
                    (208, 216, "segment_membership_block"),
                    (280, 288, "free_space_membership_block"),
                ] {
                    if bytes[flag] == 1 {
                        add_reference(&mut edges, inventory, parent, &bytes, offset, family);
                    }
                }
                ...
                let child = inventory
                    ...
                    .position(|candidate| {
                        candidate.target
                            == PhysicalArtifactReadTarget::Record(
            ...
            "root_routing_block" | "segment_membership_block" | "free_space_membership_block"
                if bytes[68] == 2 =>
            {
                let width = if granule.family == "root_routing_block" {
                    72
                } else {
                    56
                };
...
fn add_reference(
    ...
    let generation = u64_at(bytes, offset);
    let block = u64_at(bytes, offset + 8);
    let child = inventory
        .granules
        .iter()
        .position(|candidate| {
...
pub(super) fn affected(edges: &[Edge], index: usize) -> BTreeSet<usize> {
    ...
    loop {
        let old = result.len();
        for edge in edges {
            if result.contains(&edge.child) {
                result.insert(edge.parent);
            }
        }
        if old == result.len() {
            return result;
...
pub(super) fn audit(
    ...
    for index in affected.iter().copied().filter(|index| *index != target) {
        ...
        let before = std::fs::read(baseline.join(path)).unwrap();
        let after = std::fs::read(root.join(path)).unwrap();
        assert_eq!(before.len(), after.len());
        let mut allowed = BTreeSet::from_iter(44..48);
        for edge in edges
            .iter()
            .filter(|edge| edge.parent == index && affected.contains(&edge.child))
        {
            allowed.extend(edge.checksum..edge.checksum + 4);
            let child = std::fs::read(root.join(&inventory.granules[edge.child].path)).unwrap();
            assert_eq!(
                u32::from_le_bytes(after[edge.checksum..edge.checksum + 4].try_into().unwrap()),
                complete_checksum(&child)
            );
        }
        assert!(before
            .iter()
            .zip(&after)
            .enumerate()
            .all(|(offset, (a, b))| a == b || allowed.contains(&offset)));
