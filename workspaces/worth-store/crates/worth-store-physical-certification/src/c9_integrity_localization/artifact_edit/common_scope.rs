use super::{frame_checksum, ArtifactGranule, ArtifactInventory};
...
pub(super) fn substitute(
    ...
    if target.family == "inline_page" {
        let donor = inventory
            ...
            .iter()
            .find(|candidate| {
                candidate.family == "inline_page"
                    && candidate.scope.page_identity() != target.scope.page_identity()
                    && candidate.scope.page_identity().unwrap().segment_id()
                        == target.scope.page_identity().unwrap().segment_id()
                    && candidate.scope.page_identity().unwrap().page_id()
                        != target.scope.page_identity().unwrap().page_id()
...
pub(super) fn corrupt_pointer(bytes: &mut [u8], target: &ArtifactGranule) {
    ...
    if matches!(
        target.family,
        "root_routing_block" | "segment_membership_block"
    ) {
        assert_eq!(
            frame[68], 2,
            "pointer row must target a production branch, not reinterpret a leaf record key"
        );
    }
    match target.family {
        "root_manifest" | "root_routing_block" | "segment_membership_block" => {
            let parent_generation = if target.family == "root_manifest" {
                48
            } else {
                72
            };
            let value = u64::from_le_bytes(
                frame[parent_generation..parent_generation + 8]
                    .try_into()
                    .unwrap(),
            ) + 1;
