...
pub(in crate::c9_integrity_localization) fn operators(granule: &ArtifactGranule) -> Vec<Op> {
    ...
    if matches!(
        granule.family,
        "checkpoint_dirty_basis" | "checkpoint_binding" | "checkpoint_footer"
    ) {
        rows.push(Op::SelectiveAggregate);
    }
    if matches!(granule.family, "wal_frame" | "checkpoint_stream_header") {
        rows.extend([Op::Remove, Op::Duplicate]);
    }
    rows
}
...
pub(super) fn apply(
    ...
    if operator == Op::Truncate {
        ...
        wal::edit(
            &mut bytes[target.offset()..target.offset() + target.length()],
            operator,
        );
    } else {
...
pub(super) fn audit(before: &[u8], after: &[u8], target: &ArtifactGranule, operator: Op) {
    ...
    if target.grammar == FrameGrammar::Wal {
        ...
        wal::audit(
            &before[target.offset()..end],
            &after[target.offset()..end],
            operator,
        );
    } else {
