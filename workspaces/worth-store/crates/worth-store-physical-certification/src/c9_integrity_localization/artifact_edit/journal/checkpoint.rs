...
pub(super) fn edit(bytes: &mut Vec<u8>, baseline: &Path, inventory: &ArtifactInventory, index: usize, operator: Op) {
    ...
    if operator==Op::ScopeSubstitution && target.family=="checkpoint_binding" {
        ...
        let donor = inventory.granules.iter().find(|g|
            g.path==target.path && g.family=="checkpoint_binding_compaction").unwrap();
        let source = std::fs::read(baseline.join(&donor.path)).unwrap();
        assert_eq!(donor.length(),36);
        bytes.splice(start..start+length, source[donor.offset()..donor.offset()+36].iter().copied());
        length=36;
    } else {
        ...
        match operator {
            ...
            Op::Length => {
                let payload = if target.family=="checkpoint_binding" {4097} else {(length-20) as u32+1};
        ...
        if !matches!(operator,Op::CoveredByte|Op::Checksum) { seal(frame); }
    }
...
fn refresh_enclosing(bytes:&mut [u8], inventory:&ArtifactInventory, target:&ArtifactGranule, new_length:usize) {
    let delta=new_length as isize-target.length() as isize;
    let shifted=|g:&ArtifactGranule| if g.offset()>target.offset() {
    ...
    for granule in inventory.granules.iter().filter(|g|g.path==target.path) {
        ...
        let length=if granule.offset()==target.offset(){new_length}else{granule.length()};
        match granule.family {
            "checkpoint_dirty_basis"=>dirty.update(&bytes[start..start+length]),
            "checkpoint_binding"=>{bindings.update(&bytes[start..start+length]);binding_bytes+=length as u64;}
    ...
    if target.family=="checkpoint_footer" {return;}
...
fn checksum(bytes:&[u8])->u32 {
    let mut crc=!0u32;
    for byte in bytes {crc^=u32::from(*byte);for _ in 0..8 {crc=(crc>>1)^(0x82f63b78 & (crc&1).wrapping_neg());}}
    !crc
}
