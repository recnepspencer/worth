...
pub(super) fn audit(before: &[u8], after: &[u8], target: &ArtifactGranule, op: Op) {
    ...
    let new_length = if kind == 4 && op == Op::ScopeSubstitution {
        36
    } else {
        old_length
    };
    ...
    for (index, &(offset, length, record_kind)) in records.iter().enumerate() {
        let shifted = if index > selected {
            offset - old_length + new_length
        } else {
            offset
        };
        let width = if index == selected {
            new_length
        } else {
            length
        };
        ...
        match record_kind {
            ...
            3 => {
                compaction = Some((
                    shifted as u64,
                    read64(&before[offset..], 16),
                    read64(&before[offset..], 24),
                ));
            }
            4 => {
                bindings.update(frame);
                binding_count += 1;
                binding_bytes += width as u64;
            }
            _ => {}
        }
    }
    ...
    if kind != 5 {
        if matches!(op, Op::CoveredByte | Op::SelectiveAggregate) {
            assert_eq!(
                footer, pristine_footer,
                "this operator must not refresh its enclosing footer"
            );
        } else {
            for index in 0..156 {
                if !(48..88).contains(&index) && !(112..156).contains(&index) {
                    assert_eq!(
                        footer[index], pristine_footer[index],
                        "unrelated footer byte {index}"
                    );
                }
            }
        }
    ...
    assert_eq!(
        (read64(footer, 80), read64(footer, 88), read64(footer, 96)),
        (offset, generation, cutoff)
    );
    ...
    assert_eq!(
        footer[48..80] == dirty.finalize()[..],
        !false_dirty,
        "dirty selective summary isolation"
    );
    assert_eq!(
        footer[120..152] == bindings.finalize()[..],
        !false_binding,
        "binding selective summary isolation"
    );
}
...
fn target_bytes(
    ...
    new: &[u8],
    baseline: &[u8],
    records: &[(usize, usize, u8)],
    op: Op,
    kind: u8,
) {
    assert_eq!(
        valid_crc(new),
        !matches!(op, Op::CoveredByte | Op::Checksum)
    );
    if kind == 4 && op == Op::ScopeSubstitution {
        ...
        assert_eq!(
            new,
            &baseline[offset..offset + length],
            "exact lawful production donor"
        );
        return;
    }
    ...
    let field = match op {
        ...
        Op::SelectiveAggregate => match kind {
            2 => 56..57,
            4 => end - 1..end,
            5 => 48..49,
            _ => unreachable!(),
    ...
    for index in 0..new.len() {
        ...
        if !field.contains(&index) && !can_reseal {
            assert_eq!(
                old[index], new[index],
                "unrelated selected-record byte {index}"
            );
        }
    }
    match op {
        Op::CoveredByte | Op::Checksum | Op::SelectiveAggregate => {
            assert_eq!(new[field.start], old[field.start] ^ 1)
        }
        Op::Length => assert_eq!(
            read32(new, 12),
            if kind == 4 {
        ...
        Op::ScopeSubstitution => {
            assert_eq!(new[9], 4);
            assert!((1..=4096).contains(&(new.len() - 20)));
        }
        _ => unreachable!(),
    }
}
