...
pub(super) fn audit(before: &[u8], after: &[u8], operator: Op) {
    ...
    assert!(changes
        .iter()
        .all(|index| allowed.contains(index) || (operator != Op::CoveredByte && *index >= footer)));
    ...
    assert_eq!(
        payload_valid,
        !matches!(operator, Op::CoveredByte | Op::Checksum)
    );
    ...
    match operator {
        ...
        Op::ScopeSubstitution => assert_eq!(
            u64::from_le_bytes(after[12..20].try_into().unwrap()),
