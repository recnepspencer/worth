...
pub(in super::super) fn common_inspection_target(
    baseline: &Path,
    ...
    let checksum =
        enclosures::complete_checksum(&common_render(baseline, inventory, index, operator));
    ...
    let revised = if let Some(identity) = scope.root_routing_block_identity() {
        ...
        PhysicalArtifactScope::root_routing_block(
            inventory.store,
            inventory.format,
            RootRoutingBlockScopeIdentity::new(
                identity.tree(),
        ...
        PhysicalArtifactScope::segment_membership_block(
            inventory.store,
            inventory.format,
            SegmentMembershipBlockScopeIdentity::new(
                identity.tree(),
