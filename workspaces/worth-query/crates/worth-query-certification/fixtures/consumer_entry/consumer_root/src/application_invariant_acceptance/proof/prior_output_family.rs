...
use super::{length, output_correspondence::observed_source, read_y, source_version};
...
fn create_cycle(request: &super::Request<'_>, prefix: &str, command: u64) {
    create_cycle_from_points(request, prefix, command, &[(1, 1), (10, 1), (1, 10)]);
}
...
fn create_cycle_from_points(
    ...
    let outcome = super::mutate(request, input, command);
    assert!(matches!(
        outcome,
...
fn adjust_prior(request: &super::Request<'_>, offset_y: u64, command: u64) -> Vec<String> {
    let source = observed_source(request, "anchor-a");
    let outcome = request
        .mutate(PriorCycleAdjustment {
            scope_key: "anchor-a".to_owned(),
