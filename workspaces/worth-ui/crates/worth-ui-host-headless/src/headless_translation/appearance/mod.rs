...
pub(crate) fn translate(
    ...
    if source.live_host_command_count() != 0
        || source.successor().live_host_command_count() != 0
        || !source.successor().is_unpublished()
    {
        return Err(UiHeadlessAppearanceTranslationDenial::PublishedMechanic);
    }
