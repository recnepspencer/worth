...
) -> Result<(UiMountedAppearanceDelta, UiMountedAppearanceFacts), UiMountedAppearanceLoweringDenial>
{
    ...
    let order_changed = predecessor.frame().overlay_order().bottom_to_top()
        != successor.frame().overlay_order().bottom_to_top();
    ...
    .ok_or(UiMountedAppearanceLoweringDenial::WorkConstruction)?;
    let mechanics_changed = !work.changes().is_empty() || work.order_changed();
    Ok((
