...
//! the stationary viewport box, the reconciled bounds, the accepted displayed
//! offset and the region's declared chrome contract. Because the offset is the
//! accepted one, the thumb reports where the content already is, not where the
//! semantic target is heading.
...
impl UiScrollRegionChromeFacts {
    ...
    pub(in crate::facade::entry) const fn displayed_offset(
        &self,
    ) -> crate::runtime::scroll::UiScrollOffset {
        self.displayed_offset
    }
...
impl super::super::WorthUiActiveApplicationSession {
    ...
    fn region_chrome(
        ...
        // The displayed pose, not the semantic offset: the thumb reports where
